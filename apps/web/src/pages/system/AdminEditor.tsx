...
export default function AdminEditorPage() {
  ...
  return (
    <>
      ...
      {/* 보조 섹션들 — 각 섹션의 card 가 자체 mb 가 없어 형제로 붙는 문제 회피용 wrapper */}
      {isEdit && existing && (
        <div className="space-y-4">
          <LockStatusSection admin={existing} />
      ...
      {confirmCreate && (
        <ConfirmDialog
          title="관리자를 추가하시겠습니까?"
          body={`로그인 ID "${loginId}" 로 새 관리자 계정을 생성합니다.`}
          confirmLabel="예, 추가"
 ...
 */
// 잠금 정책 상수 — BE AdminLoginLogService 와 동일 (1시간/5회/1시간 잠금).
// 정책 변경 시 BE 와 함께 갱신 필요.
...
function LockStatusSection({ admin }: { admin: AdminAccount }) {
  ...
  const failCount = admin.recent_password_failure_count ?? 0
  const hasRecentFailures = failCount > 0
  ...
  // 잠금 아님 + 최근 실패도 없음 → 해제 직후 ok 메시지만 잠깐 노출하고 나머지는 숨김
  ...
  // 잠기진 않았지만 최근 실패 이력 있음 → 모니터링 배너 (해제 버튼 없음)
  if (!isLocked) {
    return (
      <div className="card text-[12.5px] border border-border">
        <div className="text-text-sub">
          최근 1시간 내 비밀번호 실패{' '}
          <strong className="text-warn-dark">{failCount}회</strong>
          <span className="text-text-soft"> / {LOCKOUT_THRESHOLD}회 임계</span>
          <span className="text-text-soft"> — 임계 도달 시 1시간 자동 잠금됩니다.</span>
        </div>
      </div>
    )
  }
    ...
    <>
      <div className="card flex items-center justify-between gap-3 border border-warn/40 bg-warn/5">
        <div className="text-[12.5px]">
          ...
          <div className="text-text-soft mt-0.5">
            최근 1시간 내 실패{' '}
            <strong className="text-warn-dark">{failCount}회</strong>
            <span> (임계 {LOCKOUT_THRESHOLD}회)</span>
          </div>
          {msg?.kind === 'err' && (
