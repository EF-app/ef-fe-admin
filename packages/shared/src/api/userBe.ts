/**
 * 백엔드 어드민 유저 관리 API 클라이언트 (단수 — /v1/admin/user).
 ...
 *
 * BE 컨트롤러: AdminUserController (/v1/admin/user)
...
interface BeUserProfile {
  mbti: string | null;
  matchPurpose: string | null;
  interestTarget: string | null;
  job: string | null;
...
// BE Purpose(LOVE/FRIEND/MIXED) → FE MatchPurpose(LOVER/FRIEND/BOTH)
function toMatchPurpose(p: string | null): MatchPurpose {
  if (p === 'LOVE') return 'LOVER';
...
function toProfile(be: BeUserProfile | null, userId: number, createTime: string): UserProfile | undefined {
  ...
  return {
    user_id: userId,
    match_purpose: toMatchPurpose(be.matchPurpose),
    ...
    interest_target: (be.interestTarget as InterestTarget | null) ?? undefined,
    keywords: {
...
function toUserDetail(be: BeUserDetail): UserDetail {
  return {
    ...
    // 제재 이력 / 매칭 / 차단 / 신고 — BE 미구현, 화면이 빈 배열 처리
  };
}
...
export const userBeApi = {
  ...
  getUserDetail: async (id: number): Promise<UserDetail> => {
    const { data } = await getApiClient().get<RspTemplate<BeUserDetail>>(
      `${BASE}/${id}`
    );
